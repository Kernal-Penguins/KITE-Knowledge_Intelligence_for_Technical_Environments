...
import { Search, Loader2, AlertTriangle, FileWarning } from "lucide-react";
import { useRunRca } from "../hooks/useKiteApi";
import MarkdownLite from "../components/MarkdownLite";

export default function RcaPage() {
  const [equipmentId, setEquipmentId] = useState("P-101");
  const rca = useRunRca();
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-white/45">
          Traverses 1-2 hop graph context for an asset and generates an LLM-written RCA report --{" "}
          <code className="text-white/70">GET /api/v1/agents/rca/{"{equipment_id}"}</code>.
        </p>
      </div>
      ...
      {rca.isError && (
        <div className="flex items-center gap-2 rounded-lg bg-[#E08A3C]/10 px-4 py-3 text-sm text-[#E08A3C] ring-1 ring-[#E08A3C]/20">
          ...
          {rca.error instanceof Error ? rca.error.message : "RCA failed."}
        </div>
      )}
        ...
        <div className="rounded-lg bg-white/[0.03] px-5 py-4 ring-1 ring-white/5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <span className="rounded-full bg-[#4FD1C5]/10 px-2.5 py-1 text-[11px] text-[#4FD1C5]">
              {rca.data.evidence_count} graph records analyzed
            </span>
          </div>
      ...
      {!rca.data && !rca.isPending && !rca.isError && (
        <p className="text-sm text-white/30">Enter an equipment tag and run the analysis.</p>
      )}
    </div>
  );
}
